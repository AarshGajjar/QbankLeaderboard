...
*   **`public/`**: Holds static assets like the main `index.html` and images (e.g., `vite.svg`).
...
### 1. `ActivityLogs.tsx`
*   **Purpose:** Displays a log of activities for two users (`user1` and `user2`), showing completed questions, correctness, and timestamps. It offers "Clock View" and "List View" visualizations, and date/user filtering.
*   **Data Sources:** Receives `logs` and `userNames` as props. `onRefresh` prop fetches new data.
*   **Key Features:**
...
*   **Overall:** A central component for activity tracking and updates, rich in display options.

### 2. `DailyProgressGraph.tsx` (ProgressDashboard)
...
### 5. `EnhancedProgress.tsx` (exports `DualUserProgress`)
...
### 6. `Heatmap.tsx` (exports `ActivityHeatmap`)
...
### 8. `RadarChart.tsx` (exports `UserStatsRadarChart`)
...
### 9. `StatsComparison.tsx` (Battle Arena)
...
*   **Key Features:**
    ...
    *   **Progress Update:** Password-protected dialog to add completed/correct questions. Calls `onUpdateProgress`. Uses `sonner` for toasts.
...
*   **Backend & Services:**
    ...
    *   **Resend:** Server-side email sending service.
